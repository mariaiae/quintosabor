...
export default function OrderForm({ dbProducts = [], user = null }) {
    ...
    const handleSubmit = async (e) => {
            ...
            if (currentUser) {
                const { data: order, error: orderError } = await supabase
                    ...
                    .insert({
                        ...
                        status: 'PENDING',
                        total_amount: total,
                        shipping_address: deliveryMethod === 'domicilio' ? address : 'Metro Hospital'
                    })
                    .select()
                    .single();
                ...
                if (order) {
                    ...
                    for (const key of Object.keys(cart)) {
                        const item = cart[key];
                        // Obtenemos el UUID consultando por nombre
        ...
        let waText = `¡Hola Quinto Sabor! Soy *${customerName}*, acabo de registrar un pedido.\n\n`;
        waText += "*Mi pedido:*\n";
        itemsList.forEach(i => waText += `- ${i.quantity} x ${i.product}\n`);
        waText += `\n*Entrega:* ${zoneText}`;
        if (deliveryMethod === 'domicilio') waText += `\n*Dirección:* ${address}`;
        waText += `\n*Cuándo:* ${deliveryDate} ${deliveryTime}`;
        waText += `\n*Método de pago:* ${paymentMethod}`;
        waText += `\n*Total a pagar:* ${formatter.format(total)}\n\n`;

        if (paymentMethod === 'Transferencia/QR') {
            waText += "Por favor, confirma el pedido y envíame el código QR para transferir. ¡Gracias!";
        } else {
            ...
            <form onSubmit={handleSubmit}>
                ...
                <section className={`form-step ${step === 3 ? 'active' : ''}`}>
                    ...
                    <p className="apple-hint" style={{ textAlign: 'center', marginTop: '15px' }}>
                        Al confirmar, continuamos con la gestión de tu pedido en nuestro WhatsApp.
                    </p>
                </section>
            </form>
        </div>
